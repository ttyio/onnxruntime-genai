...
class NemotronParseModel(Model):
    ...
    def make_genai_config(self, config, extra_kwargs, out_dir):
        ...
        # Match the native processor: decoder-start, BOS, task tokens, EOS.
        prompt_ids = processor.tokenizer.encode(self.default_user_prompt, add_special_tokens=False)
        self.prefill_sequence_length = len(prompt_ids) + 3
